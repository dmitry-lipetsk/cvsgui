...
/// Find console window data structure
typedef struct
{
...
/*!
	Callback enumerating all top-level windows and looking for console window matching given process ID and window class name
	\param hWnd Window handle
...
BOOL CALLBACK FindConsoleEnum(HWND hWnd, LPARAM lParam)
{
	...
	if( pid == data->pid )
	{
		...
		if( GetClassName(hWnd, wndClassName, CP_WNDCLASSNAME_SIZE-1) )
		{
							...
							void* appData)
{
	...
	*/
		...
		calc_command_len(cvs_process->argc, cvs_process->args) > startupInfo->commandLineLimit )
	{
		...
		if( ::GetTempPath(MAX_PATH - 1, commandFileName) && 
			::GetTempFileName(commandFileName, "cpc", 0, commandFileName) )
		{
			...
			if( vi.dwPlatformId == VER_PLATFORM_WIN32_NT )
			{
				::MoveFileEx(commandFileName, NULL, MOVEFILE_DELAY_UNTIL_REBOOT);
			}
...
int cvs_process_give_time(void)
{
	...
	// We do have to do yield to avoid 100% CPU utilization
	::Sleep(1);
	return 0;
#endif
}
