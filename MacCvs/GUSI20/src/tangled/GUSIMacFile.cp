...
static DEFINE_API_STDCALL(void) GUSIMFReadDone(ParamBlockRec * pb);
static void GUSIMFWrite(GUSIMacFileSocket * sock);
static DEFINE_API_STDCALL(void) GUSIMFWriteDone(ParamBlockRec * pb);
static DEFINE_API_STDCALL(void) GUSIMFWakeup(ParamBlockRec * pb);
...
private:
 ...
 friend DEFINE_API_STDCALL(void) 				GUSIMFReadDone(ParamBlockRec * pb);
 friend void 				GUSIMFWrite(GUSIMacFileSocket * sock);
 friend DEFINE_API_STDCALL(void) 				GUSIMFWriteDone(ParamBlockRec * pb);
 friend DEFINE_API_STDCALL(void) 				GUSIMFWakeup(ParamBlockRec * pb);
...
static DEFINE_API_STDCALL(void) GUSIMFWriteDone(ParamBlockRec * pb)
{
	GUSI_SASSERT_INTERNAL(
		!GUSIConfiguration::Instance()->fMacOSX, "We should not be there for OSX!\n");
...
static void GUSIMFWrite(GUSIMacFileSocket * sock)
{
  ...
  sock->fWritePB.ioParam.ioPosMode = fsAtMark +
  	(sock->fWriteBehind && valid >= sock->fBlockSize) ? noCacheMask : 0;
		...
		if(!GUSIConfiguration::Instance()->fMacOSX)
			PBWriteAsync(&sock->fWritePB);		
		else
		{
			PBWriteSync(&sock->fWritePB);		
			GUSI_SASSERT_INTERNAL(
				!sock->fOutputBuffer.Locked(), "We should not be there for OSX!\n");
			sock->fOutputBuffer.ClearDefer();
			sock->fOutputBuffer.FreeBuffer(
				sock->fWritePB.ioParam.ioBuffer, sock->fWritePB.ioParam.ioReqCount);
			if (sock->SetAsyncMacError(sock->fWritePB.ioParam.ioResult)) {
...
int GUSIMacFileDevice::rename(GUSIFileToken & file, const char * newname)
{
 ...
 if (file.CatInfo()->Locked()) {
  ...
  if (newexists && newfile.CatInfo()->Locked()) { // CVSGUI, was file.CatInfo()->Locked()
 	GUSIFSRstFLock(&newfile);
  	newfile.SetParID(newfile->parID); // CVSGUI, force fValidInfo = false
 	int res = rename(file, newname);
 	if (res)
	...
	: fFileRef(fileRef), fAppend(append), fWriteBehind(false)
{
 ...
 if(!GUSIConfiguration::Instance()->fMacOSX)
 {
 	if (!fReadShutdown && !sReadProc)
 		sReadProc = NewIOCompletionUPP(GUSIMFReadDoneEntry);
 	if (!fWriteShutdown && !sWriteProc)
 		sWriteProc = NewIOCompletionUPP(GUSIMFWriteDoneEntry);
 	if (!sWakeupProc)
 		sWakeupProc = NewIOCompletionUPP(GUSIMFWakeupEntry);
 }
