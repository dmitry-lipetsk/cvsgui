...
{
#if TYPE_LONGLONG && (!TARGET_API_MAC_CARBON || UNIVERSAL_INTERFACES_VERSION < 0x0334) // S64Mod is not anymore in Carbon....
...
static DEFINE_API_STDCALL(void) GUSITimerProc(TMTask * tm)
{
...
GUSITimer::GUSITimer(bool wakeup, GUSIContext * context)
{
	...
	if (wakeup) {
		if (!sTimerProc)
			sTimerProc 	= NewTimerUPP(GUSITimerProcEntry);
		tmAddr 			= sTimerProc;
	} else
		tmAddr  		= 0;
	tmCount				= 0;
