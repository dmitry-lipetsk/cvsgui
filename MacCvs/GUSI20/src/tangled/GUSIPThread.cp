...
static pascal void * GUSIThreadEntryProcWrapper(void *threadParam)
{
	CreateArg * arg = static_cast<CreateArg *>(threadParam);
		...
		const pthread_attr_t * attr, GUSIPThreadProc proc, void * arg)
{
	...
	*thread = static_cast<GUSIPThread *>(
		GUSIContextFactory::Instance()->CreateContext(
				NewThreadEntryUPP(GUSIThreadEntryProcWrapper),
