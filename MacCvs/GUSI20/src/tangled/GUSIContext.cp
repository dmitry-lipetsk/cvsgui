...
void GUSIContext::Setup(bool threading)
{
  ...
  GUSIThreadManagerProxy::Instance()->SetThreadSwitcher(
    	kApplicationThreadID, NewThreadSwitchUPP(GUSIThreadSwitchIn), sCurrentContext, true);
    GUSIThreadManagerProxy::Instance()->SetThreadSwitcher(
    	kApplicationThreadID, NewThreadSwitchUPP(GUSIThreadSwitchOut), sCurrentContext, false);
...
void GUSIContext::FinishSetup()
{
	...
	if (sHasThreading) {
		GUSIThreadManagerProxy::Instance()->SetThreadSwitcher(
			fThreadID, NewThreadSwitchUPP(GUSIThreadSwitchIn), this, true);
		GUSIThreadManagerProxy::Instance()->SetThreadSwitcher(
			fThreadID, NewThreadSwitchUPP(GUSIThreadSwitchOut), this, false);
		if (fThreadID != kApplicationThreadID)
			GUSIThreadManagerProxy::Instance()->SetThreadTerminator(
				fThreadID, NewThreadTerminationUPP(GUSIThreadTerminator), this);
	}
...
//                                                                         
// <Implementation of completion handling>=                                
GUSIContext::GUSIContext(ThreadEntryUPP threadEntry, void *threadParam, 
...
{
	if(fSwitchInProc)
		DisposeThreadSwitchUPP(fSwitchInProc);
	if(fSwitchOutProc)
		DisposeThreadSwitchUPP(fSwitchOutProc);
	if(fTerminateProc)
		DisposeThreadTerminationUPP(fTerminateProc);
...
// <Implementation of completion handling>=                                
OSErr GUSINewThread(
	ThreadStyle, ThreadEntryUPP threadEntry, void *threadParam, 
...
GUSIContext * GUSIContextFactory::CreateContext(
	ThreadEntryUPP threadEntry, void *threadParam, 
	Size stackSize, ThreadOptions options, void **threadResult, ThreadID *threadMade
) 
{
...
OSErr GUSISetThreadSwitcher(
	ThreadID thread, ThreadSwitchUPP threadSwitcher, void *switchProcParam, Boolean inOrOut)
{
	GUSIContext * context;
...
void GUSIContext::SetSwitchIn(ThreadSwitchUPP switcher, void *switchParam)
{
	fSwitchInProc = switcher;
	fSwitchInParam= switchParam;
}

void GUSIContext::SetSwitchOut(ThreadSwitchUPP switcher, void *switchParam)
{
...
OSErr GUSISetThreadTerminator(
	ThreadID thread, ThreadTerminationUPP threadTerminator, void *terminationProcParam)
{
	GUSIContext * context;
	if (!(context = GUSIContext::Lookup(thread)))
...
void GUSIContext::SetTerminator(ThreadTerminationUPP terminator, void *terminationParam)	
{
...
#endif
// <Implementation of completion handling>=                                
pascal void GUSIThreadSwitchIn(ThreadID, void *switchProcParam)
{
	GUSIContext * context = static_cast<GUSIContext *>(switchProcParam);
	context->SwitchIn();
}
...
void GUSIContext::SwitchIn()
{
	...
	if (fSwitchInProc)
		InvokeThreadSwitchUPP(fThreadID, fSwitchInParam, fSwitchInProc);
}
...
//                                                                         
// <Implementation of completion handling>=                                
pascal void GUSIThreadSwitchOut(ThreadID, void *switchProcParam)
{
	GUSIContext * context = static_cast<GUSIContext *>(switchProcParam);
	context->SwitchOut();
}
...
{
	if (fSwitchOutProc)
		InvokeThreadSwitchUPP(fThreadID, fSwitchOutParam, fSwitchOutProc);
	fErrno		=	errno;
	fHostErrno	=	h_errno;
...
// <Implementation of completion handling>=                                
pascal void GUSIThreadTerminator(ThreadID, void *terminationProcParam)
{
	GUSIContext * context = static_cast<GUSIContext *>(terminationProcParam);
	context->Terminate();
}
...
void GUSIContext::Terminate()
{
	...
	if (!(fFlags & asleep))
		--fProcess->fReadyThreads;
	if (fTerminateProc)
		InvokeThreadTerminationUPP(fThreadID, fTerminateParam, fTerminateProc);
	if (fFlags & detached) {
		delete this;
	} else {
		fFlags |= done;
...
static DEFINE_API_STDCALL(void) GUSIIODone(ParamBlockRec * pbrec)
{
	IOParam * pb = reinterpret_cast<IOParam *>(pbrec);
	if (Context(pb)) 
		Context(pb)->Wakeup();
}
...
void GUSIStartIO(IOParam * pb)
{
	...
	if (!sIODone)
		sIODone = NewIOCompletionUPP(GUSIIODoneEntry);
	Context(pb) 		= nil;
...
OSErr GUSIThreadManagerProxy::NewThread(
		ThreadStyle threadStyle, ThreadEntryUPP threadEntry, void *threadParam, 
...
OSErr GUSIThreadManagerProxy::SetThreadSwitcher(ThreadID thread, 
		ThreadSwitchUPP threadSwitcher, void *switchProcParam, Boolean inOrOut)
{
...
OSErr GUSIThreadManagerProxy::SetThreadTerminator(ThreadID thread, 
		ThreadTerminationUPP threadTerminator, void *terminationProcParam)
{
