...
#define OPERATOR_ARRAY_NEW		_std_operator_array_new
#define OPERATOR_ARRAY_DELETE	_std_operator_array_delete

void* _std_operator_new(PP_STD::size_t size) throw(PP_STD::bad_alloc);
void* _std_operator_new(PP_STD::size_t size, const PP_STD::nothrow_t&) throw();
void _std_operator_delete(void* ptr) throw();

void *_std_operator_array_new(PP_STD::size_t size) throw(PP_STD::bad_alloc);
void *_std_operator_array_new(PP_STD::size_t size,const PP_STD::nothrow_t&) throw();
void _std_operator_array_delete(void *ptr) throw();
...
typedef void* (*_op_new)(PP_STD::size_t);
typedef void* (*_op_new_nothrow)(PP_STD::size_t, const PP_STD::nothrow_t&);
...
AllowAllocation()
{
	return ((UMiscUtils::MemoryIsLow() == false)
			&& (StCriticalSection::InCriticalSection() == false)
			&& (StInterruptSection::InInterruptSection() == false));
}
	...
	PP_STD::size_t	size) throw (PP_STD::bad_alloc)
{
	...
	if (AllowAllocation()) {
		...
		// The allocation function should throw std::bad_alloc if
		...
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, _std_operator_new, false);
		#else
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, (_op_new)_std_operator_new, false);
		#endif
	#else
		ptr = _std_operator_new(size);
	#endif
	}
...
#pragma unused(nt)	// Keeps the compiler quiet
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, _std_operator_new, nt, false);
		#else
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, (_op_new_nothrow)_std_operator_new, false);
		#endif
	#else
		ptr = _std_operator_new(size, nt);
	#endif
	}
	...
	PP_STD::size_t	size) throw (PP_STD::bad_alloc)
{
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, _std_operator_array_new, true);
		#else
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, (_op_new)_std_operator_array_new, true);
		#endif
	#else
		ptr = _std_operator_array_new(size);
	#endif
	}
...
#pragma unused(nt)	// Keeps the compiler quiet
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, _std_operator_array_new, nt, true);
		#else
		ptr = DebugNewDoAllocate(size, (const char *)0, 0, (_op_new_nothrow)_std_operator_array_new, true);
		#endif
	#else
		ptr = _std_operator_array_new(size, nt);
	#endif
	}
...
#pragma unused (file, line)	// Quiet down the non-debug builds.
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, file, line, _std_operator_new, false);
		#else
		ptr = DebugNewDoAllocate(size, file, line, (_op_new)_std_operator_new, false);
		#endif
	#else
		ptr = _std_operator_new(size);
	#endif
...
#pragma unused (nt, file, line)	// Quiet down the non-debug builds.
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, file, line, _std_operator_new, nt, false);
		#else
		ptr = DebugNewDoAllocate(size, file, line, (_op_new_nothrow)_std_operator_new, false);
		#endif
	#else
		ptr = _std_operator_new(size, nt);
	#endif
...
#pragma unused (file, line)	// Quiet down the non-debug builds.
	...
	#if PP_Debug && PP_DebugNew_Support && (DEBUG_NEW >= DEBUG_NEW_BASIC)
		#if __MSL_CPP__ > 0x5300
		ptr = DebugNewDoAllocate(size, file, line, _std_operator_array_new, true);
		#else
		ptr = DebugNewDoAllocate(size, file, line, (_op_new)_std_operator_array_new, true);
		#endif
	#else
		ptr = _std_operator_array_new(size);
	#endif
	...
	void*	ptr)
{
	...
	StSpotlightDisable_();
	DebugNewDoFree(ptr, _std_operator_delete, false);

#else

	_std_operator_delete(ptr);
	...
	void*	ptr)
{
	...
	StSpotlightDisable_();
	DebugNewDoFree(ptr, _std_operator_array_delete, true);

#else

	_std_operator_array_delete(ptr);
