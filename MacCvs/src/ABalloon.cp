			...
			theWindow->IsEnabled() )
		{
			...
			while (iterator.Next(theAttachment))
			{
				...
				if ( (ownerPane != nil) &&
					...
					(ownerPane->GetMacWindow() == macWindowP) &&
#else
...
void	ABalloon::ShowBalloon( SInt16 inStringIndex, Rect& inAltRect )
{
	...
	else
	{
		...
		::HMShowBalloon( &theMessageRec, ComputeTip( inAltRect ),
			&inAltRect, nil, 0, 0, kHMSaveBitsWindow );
	}
#endif // !qCarbon
}
...
/*	---------------------------------------------------------------------------------------------
	AUMBalloon::ComputeTip
	
	Forces the balloon to be shown right under the mouse.
	The mouse location is used instead of calculating the position.
	---------------------------------------------------------------------------------------------
*/
Point	AUMBalloon::ComputeTip( const Rect& inAltRect )
{
	Point	theTip;
	//theTip.v = inAltRect.top;	//removes a warning
	
	::GetMouse(&theTip);
	/*
	LPane*	thePane = dynamic_cast<LPane*>( mOwnerHost );
	thePane->PortToGlobalPoint( theTip );
	*/
	theTip.h += inAltRect.left;
	theTip.v += inAltRect.top;
	return theTip;
}
