...
INT_PTR CCJToolBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	INT_PTR nHit = CCJToolBarBase::OnToolHitTest(point, pTI);
	if (nHit != -1)
		return nHit;
...
#if 1
// [2012-11-08] sf_animal: Dirty hack for VS2010. Amen.

#if (_MSC_VER < 1200) // less that VC6

	ASSERT(m_hbmImageWell == NULL ||
		( ::GetObjectType(m_hbmImageWell) == OBJ_BITMAP));

#elif (_MSC_VER < 1600)	// less that VS2010

	ASSERT(m_hbmImageWell == NULL ||
		( afxData.bWin95 || ::GetObjectType(m_hbmImageWell) == OBJ_BITMAP));

#else //VS2010 and later [afxData.bWin95 not defined]
